...
// 🏈 Map of NFL teams to their subreddit names
...
function TeamTalk() {
  ...
  // 💾 Save favorite team in localStorage
  const saveFavoriteTeam = useCallback((teamName) => {
    localStorage.setItem("favoriteTeam", teamName);
  }, []);

  // 📦 Load favorite team from localStorage
  const loadFavoriteTeam = useCallback(() => {
    return localStorage.getItem("favoriteTeam") || "Ravens";
  }, []);

  // 🚀 Fetch subreddit posts for selected team
  const fetchPosts = useCallback(
    async (teamName, isRefresh = false) => {
      ...
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_BASE_URL}/reddit/team/${encodeURIComponent(
            teamName
          )}`,
          { timeout: 10000 }
        );
    ...
    [] // ✅ No external dependencies because it uses only local state setters
  );

  // 🎯 Load the favorite team and fetch posts on first render
  useEffect(() => {
    const favoriteTeam = loadFavoriteTeam();
    setSelectedTeam(favoriteTeam);
    setTeam(favoriteTeam);
    fetchPosts(favoriteTeam);
  }, [fetchPosts, loadFavoriteTeam, setTeam]); // ✅ added proper deps

  // 🔄 When selected team changes, refetch posts
  useEffect(() => {
  ...
  }, [selectedTeam, team, setTeam, saveFavoriteTeam, fetchPosts]); // ✅ cleaned up deps

  const handleTeamChange = (newTeam) => setSelectedTeam(newTeam);
  const handleRetry = () => fetchPosts(selectedTeam);
